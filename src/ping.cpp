...
public:
    ...
    PingTarget(const std::string& target, int timeout, std::function<void(int)> cb) :
        ...
        // Use fixed ID 256 which becomes 0x0001 in network byte order (big-endian on wire)
        // This is for compatibility with ArtNet controllers that don't properly implement ICMP
        // and always respond with ID 0x0001 regardless of the request ID
        icp->icmp_id = 256;
